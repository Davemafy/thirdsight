...
export function parseBrowserObservation(input: unknown): BrowserObservationV1 {
  ...
  if (input.schemaVersion !== "browser-observation.v1") {
    throw new BrowserObservationValidationError(
      "Unsupported browser observation schema version.",
    );
  }
...
export function projectBrowserObservationToEvidenceGraph(
  input: unknown,
): EvidenceGraphRecord {
  ...
  const originRelationship = pageOrigin
    ? pageOrigin === destinationOrigin
      ? "SAME_ORIGIN"
      : "CROSS_ORIGIN"
    : "UNKNOWN";

  const observedProvenance: EvidenceRef = {
    source: "browser",
    sourceId: `${observation.sensorId}:${observation.observationId}`,
    observedAt: observation.observedAt,
    confidence: "OBSERVED",
  };

  const lowerBoundProvenance: EvidenceRef = {
    ...observedProvenance,
    confidence: "OBSERVED_LOWER_BOUND",
  };

  const did: RuntimeAccessEvidence = {
    boundary: "browser",
    phase: "ATTEMPTED",
    pageOrigin,
    destinationOrigin,
  ...
  const could: BrowserCapabilityLowerBound = {
    ...
    destinationOrigin,
    statement:
      "The browser runtime executed an outbound request toward this destination. This is only a lower bound on technical capability and does not establish the complete permission surface.",
  };
  ...
  return {
    ...
    should: unknownClaim(
      "No merchant Purpose Contract is supplied by passive browser observation.",
    ),
    could: {
      status: "PARTIAL",
      confidence: "OBSERVED_LOWER_BOUND",
      value: could,
      provenance: [lowerBoundProvenance],
      reason:
        "Observed request execution proves only a browser-visible lower bound, not the integration's full technical access surface.",
    },
    did: {
      status: "KNOWN",
      confidence: "OBSERVED",
...
function unknownClaim<T>(reason: string): EvidenceClaim<T> {
  return {
    status: "UNKNOWN",
    confidence: "UNKNOWN",
    value: null,
...
function readNonEmptyString(
  ...
  if (typeof value !== "string" || value.trim().length === 0) {
    throw new BrowserObservationValidationError(`${key} must be a non-empty string.`);
  }
  return value;
}
...
function readBoolean(record: Record<string, unknown>, key: string): boolean {
  ...
  if (typeof value !== "boolean") {
    throw new BrowserObservationValidationError(`${key} must be a boolean.`);
  }
  return value;
}
...
function readIsoTimestamp(
  ...
  if (Number.isNaN(Date.parse(value))) {
    throw new BrowserObservationValidationError(`${key} must be an ISO timestamp.`);
  }
  return value;
}

function readHttpUrl(record: Record<string, unknown>, key: string): string {
  const value = readNonEmptyString(record, key);
  assertHttpUrl(value, key);
  return value;
}

function readNullableHttpUrl(
  record: Record<string, unknown>,
  key: string,
): string | null {
  const value = record[key];
  ...
  if (typeof value !== "string" || value.trim().length === 0) {
    throw new BrowserObservationValidationError(
      `${key} must be null or a non-empty http(s) URL.`,
    );
  }
  assertHttpUrl(value, key);
  return value;
...
function assertHttpUrl(value: string, key: string): void {
  ...
  try {
    ...
    throw new BrowserObservationValidationError(`${key} must be a valid URL.`);
  }
