...
export interface IntegrationOriginBinding {
  bindingId: string;
  integrationId: string;
  origin: string;
  environment: string;
  confidence: IntegrationBindingConfidence;
  sourceId: string;
  validFrom: string;
  validTo: string | null;
}
...
export interface IntegrationResolutionResult {
  ...
  confidence: IntegrationBindingConfidence | "UNKNOWN";
...
export function resolveIntegrationIdentity(
  evidence: EvidenceGraphRecord,
  bindings: readonly IntegrationOriginBinding[],
  environment: string,
): ResolvedEvidenceGraph {
  const destinationOrigin = evidence.did.value?.destinationOrigin;
  if (!destinationOrigin) {
    return unresolved(evidence, "No observed destination origin is available for identity resolution.");
  }
  ...
  const matching = bindings.filter((binding) => {
    ...
    if (binding.validTo !== null) {
      const validTo = Date.parse(binding.validTo);
      if (Number.isNaN(validTo) || validTo <= observedAt) return false;
    }
  ...
  if (matching.length === 0) {
    return unresolved(
      evidence,
      `No active exact-origin binding exists for ${normalizedDestination} in ${environment}.`,
    );
  }
  ...
  if (integrationIds.length > 1) {
    return {
      ...
      resolution: {
        ...
        integrationId: null,
        confidence: "UNKNOWN",
        bindingIds: matching.map((binding) => binding.bindingId),
        sourceIds: matching.map((binding) => binding.sourceId),
        reason:
          "Multiple active exact-origin bindings point to different integrations; ThirdSight refuses to guess ownership.",
      },
    };
  }

  const integrationId = integrationIds[0];
  const strongest = matching.some((binding) => binding.confidence === "AUTHORITATIVE")
    ? "AUTHORITATIVE"
    : "DECLARED";
  ...
  return {
    ...
    resolution: {
      ...
      sourceIds: matching.map((binding) => binding.sourceId),
      reason:
        strongest === "AUTHORITATIVE"
          ? "Resolved by an active exact-origin merchant registry binding."
          : "Resolved by an active exact-origin declared binding; ownership remains declared rather than independently proven.",
    },
  };
}
